...
class ImageTrailVariant1 {
  constructor(container) {
    ...
    this.isIdle = true;
    this.threshold = 120; // Distance threshold - higher means images appear less frequently
    this.lastImageTime = 0;
    this.imageCooldown = 800; // Minimum time between images (ms) - higher means slower changes
    this.currentActiveImage = null; // Track the currently active image
  ...
  render() {
    ...
    const timeSinceLastImage = currentTime - this.lastImageTime;

    // Only show next image if enough distance moved AND cooldown has passed
    if (distance > this.threshold && timeSinceLastImage >= this.imageCooldown) {
      this.showNextImage();
  ...
  showNextImage() {
    // Hide the currently active image first
    if (this.currentActiveImage && this.currentActiveImage.DOM && this.currentActiveImage.DOM.el) {
      gsap.to(this.currentActiveImage.DOM.el, {
        duration: 0.4,
        ease: 'power2',
        opacity: 0,
        scale: 0,
        onComplete: () => {
          gsap.set(this.currentActiveImage.DOM.el, {
            x: -9999,
            y: -9999,
            opacity: 0,
            scale: 0
    ...
    this.currentActiveImage = img; // Track the new active image
...
export default function ImageTrail({ items = [], variant = 1 }) {
  ...
  const encodeImageUrl = (url) => {
    ...
    try {
      ...
      // Encode only the filename to handle special characters
      const encodedFilename = encodeURIComponent(filename);
      return path ? `${path}/${encodedFilename}` : encodedFilename;
    } catch (e) {
      return url;
    }
  };
      ...
      {items.map((url, i) => {
        ...
        return (
          <div className="content__img" key={i}>
            <div className="content__img-inner" style={{ backgroundImage: `url(${encodedUrl})` }} />
          </div>
        );
      })}
    </div>
